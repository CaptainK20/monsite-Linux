...
abstract class GlossifyBase extends FilterBase implements ContainerFactoryPluginInterface {
   ...
   */
  protected function parseTooltipMatch($text, array $terms, $case_sensitivity, $first_only, $displaytype, $tooltip_truncate, $urlpattern, $ignore_tags, $langcode) {
    ...
    // Glossified terms already wrapped in <a> or <abbr>, should be considered
    // as the first occurence, so we consider them as already handled:
    if ($first_only) {
      foreach ($xpath->query('//a | //abbr') as $linkNode) {
        foreach ($pattern_parts as $pattern_part) {
          preg_match_all($pattern_part, $linkNode->nodeValue, $matches_part, PREG_OFFSET_CAPTURE);
          if (count($matches_part[0])) {
            foreach ($matches_part[0] as $match_part) {
              // We found a link containing this term so consider it already
              // handled:
              $replaced[] = $match_part[0];
            }
          }
        }
      }
    ...
    $ignore = '';
    if (strlen((string) $ignore_tags)) {
      $ignore_tags_array = explode(',', preg_replace('/[^a-z0-9,]/', '', strtolower($ignore_tags)));
      foreach ($ignore_tags_array as $tag) {
        $ignore .= " | ancestor::$tag";
      }
    }

    // Process the HTML ignoring <a> and <abbr> tags, so they don't get wrapped
    // twice in abbr / anchor tags:
    $text_nodes = $xpath->query('//text()[not(ancestor::a | ancestor::abbr' . $ignore . ') and not(ancestor::*[contains(concat(" ",normalize-space(@class)," ")," glossify-exclude ")])]');
      ...
      if (count($matches) > 0) {
        ...
        foreach ($matches as $char_offset => $match) {
          ...
          if ($current_path == str_replace('[id]', $term->id, $urlpattern)) {
            // Reinsert the found match if whe are on the page this match points
            // to.
          ...
          elseif ($first_only && in_array(strtolower($term_txt), array_map('strtolower', ($replaced)))) {
            ...
            if ($displaytype == 'links' || $displaytype == 'tooltips_links') {
              ...
              $replacementTextOrHtml = $this->renderLink($word_link);
            }
            else {
              // Has to be 'tooltips'.
              ...
              $replacementTextOrHtml = $this->renderTip($word_tip);
            }
          ...
          // of the DOM class, only appendXML (which is not appropriate and
          // may cause warnings if we are inserting invalid XML chars like
          // &nbsp;.
          $tmpDoc = Html::load($replacementTextOrHtml);
          foreach ($tmpDoc->getElementsByTagName('body')->item(0)->childNodes as $node) {
            $node = $html_dom->importNode($node, TRUE);
