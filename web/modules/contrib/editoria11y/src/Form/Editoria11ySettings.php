...
class Editoria11ySettings extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    ...
    $form['text'] = [
      '#markup' => '<h2>Getting started</h2><ol><li>Make sure ' . $linkToPermissions . ' have the "View Editoria11y checker" permission, and assign permissions to "mark OK" as appropriate.</li><li>To ' . $linkToDashboard . ', users need "Manage Editoria11y results." For the link to appear in their admin toolbar, they would also need "View site reports."</li><li>Remember that only saved pages sync their results to the dashboard; the in-editor checker only syncs dismissals.</li><li>If the checker <strong>toggle</strong> does not appear: make sure a z-indexed or overflow-hidden element in your front-end theme is not hiding or covering the <code><em>ed11y-element-panel</em></code> container, make sure that any custom selectors in the "Disable the scanner if these elements are detected" field are not present, and make sure that no JavaScript errors are appearing in your <a href="https://developer.mozilla.org/en-US/docs/Tools/Browser_Console">browser console</a>.</li><li>If the checker toggle is present but <strong>not detecting</strong> errors, or missing errors that should be flagged: check that your inclusions & exclusion settings below are not missing or ignoring the elements. It is not uncommon for homepages or views to insert editable content outside the <code><em>main</em></code> element.</li></ol><p><a href="https://www.drupal.org/project/editoria11y">Project overview</a> | <a href="https://jjameson.mycpanel.princeton.edu/editoria11y/">Working Demo</a> | <a href="https://www.drupal.org/project/issues/editoria11y?categories=All">Issue queue</a></p>',
    ];
    ...
    $form['setup']['headings'] = [
      ...
      '#markup' => $this->t('<p>To check headings in CKEditor, Editoria11y needs to know what the first heading level should be in this field. Body fields should generally be at the h2 level.</p>'),
    ];
    ...
    $form['adv']['tests'] = [
      '#type' => 'details',
      '#title' => t('Custom components and tests'),
    ];
